...
if ( !function_exists('st_fetaure_slider_array')  ) :
 ...
 function st_fetaure_slider_array($from_slider)
 {
    ...
    {
        $st_blog_fs_posts = salient_customizer_get_repeated_all_value(7,$reapeted_page);
    ...
    if ( !empty($st_blog_feature_slider_args) )
    {
        ...
        if ( $st_blog_fs_query->have_posts() ) :
            ...
            while( $st_blog_fs_query->have_posts() ) :
                ...
                {
                    $thumb = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_id() ), 'feature-slider-image');
                    $thumb_image = $thumb['0'];
                }
